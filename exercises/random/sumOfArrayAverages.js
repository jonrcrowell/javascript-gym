...
const answer = sumAverage([[3, 4, 1, 3, 5, 1, 4], [21, 54, 33, 21, 77]]);
console.log("TCL: answer", answer);

function sumAverage(arrays) {
  const averageMe = arr => arr.reduce((a, b) => a + b) / arr.length;
  return Math.floor(arrays.map(averageMe).reduce((a, b) => a + b));
}
